...
graph.registry; // Frontier registry graph shared with DOM, Playwright, and logging.
...
Actions can also call other registered actions while preserving the runtime edge:
...
actions.register({
  ...
  calls: ['audit.add'],
  run(ctx) {
    ctx.call('audit.add', { message: 'toggle dispatched' });
  }
});

const impact = actions.impact({ paths: ['/todos/a/done'] });
```
...
Latest local package benchmark on Node v26.1.0, darwin arm64, 80 rounds:

| Fixture | Compile median | Apply median |
| --- | ---: | ---: |
| 1% sparse selector update | 2.63 ms | 3.37 us |
| Indexed id `in` selector update | 1.56 ms | 3.69 us |
| 10% dense selector update | 2.76 ms | 17.16 us |
| Repeated arithmetic fold, 1000x | 0.43 us | 0.04 us |
| Transaction defer deletes, 64 fields | 73.59 us | 8.43 us |
| Frame validate, 32 watched paths | 0.81 us | 0.00 us |
| Action dispatch, query + patch commit | 9.32 us | 0.00 us |
| Repeated text append fold, 1000x | 0.75 us | 0.10 us |
